...
function SensorCard(props) {
  ...
  useEffect(() => {
    if (props.login == "Client" || props.login == "Admin") {
      ...
      axios
        .post("https://server-curtain.onrender.com/device-select", props)
        .then((res) => {
          setstat([]);
          setstatcpy([]);
          ...
          res.data.map((ele) => {
            axios
              .post("https://server-curtain.onrender.com/find", {
                id_view: ele["uniqueId"],
              })
              .then((resu) => {
                if (resu.data[0] != null) {
                  axios
                    .post("https://server-curtain.onrender.com/checki", {
                      id: ele["uniqueId"],
                    })
                    .then((nres) => {
                      setstat((prev) => {
      ...
      .then((res) => {
        if (res.data == "ON") {
          if (event.target.checked == false) {
            axios.post("https://server-curtain.onrender.com/change", {
              id: event.target.id,
              st: 0,
            });
          } else {
    ...
    <>
      <div class="table-wrapper table-striped">
        <div class="table-title">
          <div class="row">
            <div class="col-sm-3">{/* <h2>Filtered Result</h2> */}</div>
            <div class="col-sm-9">
                ...
                <tr data-status="active" key={index + 1}>
                  ...
                  <td>
                    <span
                      ...
                      className={
                        "label label-" +
                        (ele[Object.keys(ele)[0]][0]["Power"] > 1000
                          ? "danger"
                          : ele[Object.keys(ele)[0]][0]["status"] == "ON"
                          ? "success"
                          : "warning")
                      }
                    >
